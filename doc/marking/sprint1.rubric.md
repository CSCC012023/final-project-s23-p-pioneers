...
## User Stories (Tracked in JIRA) (max 10 marks) 
  ...
  - Broken down into tasks/subtasks (max 2 marks)
    ...
    Your Mark: 1 (didn't break down into tasks/subtasks) 
  ...
  - Estimated (max 2 marks)
    ...
    Your Mark: 0 (didn't record estimate in Jira)
  ...
  - Clearly Defined Acceptance Criteria (max 2 marks)
    ...
    Your Mark: 0 (didn't define acceptance criteria in Jira) 
...
## Tracking on JIRA  (max 20 marks) 
  ...
  - Do the pull request on GitHub match the tasks listed on JIRA Active Sprint Board? (max 2 marks)
    ...
    Your Mark: 1 (Tasks are grouped together, but no reference to JIRA tickets in PRs) 
...
Tips to improve for next sprints:

- [ ] Write acceptance criteria and story point estimates in JIRA
  - You only need it for the top level story, not the subtasks

- [ ] Add JIRA ticket numbers to PRs, commit messages, and branch names
  - Keep it small and consistent: One ticket (either task or subtask), one branch, and (try for) one PR
  - A nice standard structure for commit messages: `[COB-XXX] feat: messages here`
  - Branch names should include the ticket number 'somewhere': `feat/COB-XXX-new-feature`

- [ ] When a story is being worked on, its usually good to create a subtask just for implementation
  - You can have another subtask for testing, but thats optional
  - In the event that a story needs polishing, you can just create a subtask for that
  - ... Or if its a bigger refactor, make a brand new story, and reference the original story in the description
    - Rather than moving the old story to the next sprint